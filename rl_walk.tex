...
Again, on every tick the current state is defined, the next action is looked up from the policy, and the behaviour below occurs. It is important to note that some other updates and calculations are also performed depending on which goal is required (for example, lifting up the legs when necessary).
...
%/* Reads in an RL file.
% * Arguments:
% *    plannerFile -> string name of the rl file
% *
% * The expected format of an RL file is as follows:
% * x1 x2 x3 ... xn action
% * where:
% *    x1 .. xn are the state variables
% *    action is the action to be taken given all of the above
% *
% * Note: Lines beginning with // or blank lines are ignored.
% * Note: the first line of the policy MUST be the multiple line.
% *       If x1..xn are non-integers, we divide them by a multiple
% *       so that we get an integer multiple. For example, if
% *       x1 has values 0.3, 0.6, 0.9, 1.2, etc., then we set its
% *       multiple value to be 0.3. We assume there is a 0 state.
% *       Thus, the multiple line must consist of a multiple value
% *       for all x1 .. xn columns.
% */
%bool RLPlanner::loadFile(std::string plannerFile) {
%   llog(INFO) << "Loading " << plannerFile.c_str() << endl;
%   ifstream f(plannerFile.c_str());   
%   if (!f.is_open()) return false;
%
%   // Get each of the columns in the first line and store them
%   // into the multiples vector.
%   string line = "";
%   while((line = getLineNoComments(f)) == "");
%   stringstream s(line);
%   for (double tmp; s >> tmp; multiples.push_back(tmp));
%
%   // Go through each line in the policy file
%   while (f.good()) {
%      if ((line = getLineNoComments(f)) == "") continue;
%      stringstream s(line);
%
%      // Grab all the state columns, and put them into an
%      // integer state vector.
%      vector<int> state;
%      for (int i = 0; i < (int)multiples.size(); i++){
%          double tmp;
%          s >> tmp;
%          state.push_back(round(tmp / multiples.at(i)));
%      }
%
%      // Get the action, which should be the last column
%      Action action;
%      s >> action;
%
%      // Finally, store the state vector into the policy map.
%      policy[state] = action;
%   }
%   f.close();
%   llog(INFO) << "Finished loading." << endl;
%   return true;
%}
%
...
%}
%
%/* Gets the action given the state variables, in the same order
% * as those in the policy table. Assumes state variables are floats,
% * and divides them by their multiple as required, such that they become
% * the nearest integer index.
% *
% * Note: ARGUMENTS *MUST* BE FLOATS/DOUBLES
% *
% * use as such: 
% *     Action a = getAction(x1, x2, x3, ... xn);
% */
%Action RLPlanner::getAction(double x, ...) {
%   // Store the state variables (i.e. arguments) in a vector
%   vector<int> state;
%
%   // Push the first argument on, since it's not in the va_list
%   // printf("%f -> ", x);
%   state.push_back(round(x / multiples.at(0)));
%
%   // Push the rest of the arguments on to the state vector
%   va_list vl;
%   va_start(vl, x);
...
%
%   try{
%       // Get the next action based on the state in the policy table.
%       // printf("%d\n", policy.at(state));
%       return policy.at(state);
%   } catch (const std::out_of_range& e){
%       // Couldn't find anything close to that state in the policy table,
%       // so return an invalid action.
